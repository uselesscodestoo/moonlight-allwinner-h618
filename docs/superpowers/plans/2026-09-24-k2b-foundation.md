...
- [x] 模拟依赖安装，确认不移除包、不更换内核。实际安装只选构建所需包，不执行 dist-upgrade。
- [x] 用包含子模块的源码归档传入此前不存在的 `/home/kickpi/projects/moonlight-embedded`；保留两个探针项目。归档不包含 `.git`、日志或运行库秘密配置。
- [x] 在板端记录版本并配置原有 SDL 构建，不启动视频：
...
- [x] 先写测试与可编译的最小拒绝桩；运行有效 1920×1088 分配/1920×1080 裁剪案例，观察断言失败，而不是编译报错。
...
- [x] 补充每项仅修改一个约束的反例：NULL、负 fd、非 NV12、非零 Y 起点、
  UV 起点与 stride×存储高不一致、短分配、奇数 stride/存储高/裁剪、
  裁剪越界、非 1920×1080、非法颜色枚举、超大字段。
- [x] 增加有效例：1920×1080 紧凑分配、2048 步长、偶数非零裁剪且分配足够、
...
- [x] Makefile 用 `-std=c99 -O2 -Wall -Wextra -Werror` 编译真实测试和 `frame.c`。
  `make -f tests/k2b/Makefile test` 在主机和板端均须以 0 退出。
- [x] 用只运行测试的 ASan/UBSan 构建覆盖边界输入（支持该工具的 Linux 环境）。
- [x] 完成四文件提交与两阶段独立审查，不接入设备、不导出虚假的 k2b 平台能力。

## Task 3：记录基线与下一阶段接入点

- [x] `docs/k2b-bringup.md` 记录固定子模块、构建命令、测试结果与可复现环境。
- [x] 明确 `frame_validate` 不证明像素正确、不证明 fd 有效、不证明显示完成，
  它只是生产路径将复用的布局前置检查。
- [x] 保持原分支 `4e870a2` 不动，核对本分支变更范围与原有脚本语法。
- [x] 对实现做独立规格审查和质量审查，修复实际问题后才推进后续组件。
- [x] 在本计划中更新完成项，保留完整串流目标未完成状态。

阶段结果：`acfd079` 布局实现与测试，`5e8bebb` 补充接口单位/所有权注释。
普通/NDEBUG 主机测试均 59 项通过；板端普通/NDEBUG/ASan+UBSan 各 59 项通过。
板端在用户移除热点前同步至 `acfd079`。此后的本地提交暂未同步，日志回传
因网络中断未完成；板端日志文件已有成功输出，不将网络断开视作板卡崩溃。
