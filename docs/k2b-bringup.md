...
`f08f389` 加载器通过本地三种构建与板端普通/NDEBUG 各 242 场景单测。
`39cff35` / `2557b17` 接入构建和受控启动后，已在普通板端用户下真实加载
九个 Cedar 库、核对兼容层并注册硬件 H.264；缺少环境的负例明确失败。
独立与集成 ON 两套原生产物的 strace 都通过无设备打开/无 ioctl 检查，
内存状态全零，OFF 基线构建也通过。板端代码为 `2557b17`，未重启、未操作
VPU/HDMI。详细命令、日志、审查结果和路径限制见
[私有运行库记录](k2b-cedarc-runtime.md#真实私有加载与注册已验证)。

下一步是把生产解码工作线程接入这份真实函数表；图片持有、显示退役和
实际 1920×1080、60 个不同帧/秒的目标仍未验收。

## 校园网地址与解码生命周期调查

2026-09-24：用户将板卡接入校园网，新地址为 `kickpi@172.31.197.223`。
板卡未认证外网；开发使用主机已有源码和 SSH 传输，不依赖板卡联网下载。
该地址已实际登录：内核仍为 5.4.125，启动 ID 为
`66ed4017-1fae-4b40-82ca-877d257b5175`，板端源码 `89c5fbf` 且工作树干净。
检查时 CMA 总量 131072 kB、可用 78068 kB，测试堆节点不存在。
这次没有重启、加载模块、初始化 VPU 或改变 HDMI。

已用固定二进制反汇编确认 SBM 的析构调用在 `VideoEngineDestroy` 中，
