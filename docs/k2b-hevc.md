...
Branch: k2b-hevc, starting at the validated AVC/desktop-restore commit 6bdfe09.
Stable k2b-cedarc-disp and its board checkout remain untouched.

## Run on the board

```sh
sudo sh ~/projects/moonlight-hevc/tools/k2b-stream.sh 172.31.193.248 -codec hevc
```

The extra codec option overrides the script's H.264 default. Without it this
branch still defaults to AVC. Ctrl+Shift+Alt+Q exits normally; the existing
systemd post-hook restores XFCE. Pairing was copied locally into this worktree's
ignored build/k2b-pairing directory; no private key is committed.
...
Board evidence in this worktree's build directory:
